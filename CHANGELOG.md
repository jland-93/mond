...
로그인 화면 히어로와 브랜드 아이콘 다듬기. 기능 변경 없음(UI/자산).

### Changed
- **로그인 3D 달 리얼리즘** — 캔버스 절차생성 텍스처 + 공전 조명(초승달이 빙빙 도는 어색함)을 걷어내고, 실사 달 텍스처(lunar color map) + bump로 분화구 요철, 고정 조명의 은은한 명암(terminator), 차분한 자전으로 교체. 정사각 캔버스 모서리는 원형 마스크로 잘라 배경의 사각 틀 제거.
- **브랜드 마크 정리** — 로그인 상단 로고를 배경/텍스트가 박힌 앱 아이콘 대신 배경 투명 초승달 마크(`logo-mark.png`)로 교체.
- **파비콘 세트 일원화** — 텍스트 없는 초승달을 네이비 브랜드 타일에 얹은 정사각 세트(`favicon.ico` + 16/32/96 PNG + apple-touch 180 + PWA 192/512 maskable)로 재구성하고 `index.html`·`site.webmanifest` 재연결. 비정사각·미참조 구 아이콘 정리.
