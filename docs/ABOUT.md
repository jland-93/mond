...
- [x] **UX·가시화 보강** — IAM Explorer/Access Center/Access Review에서 ARN·UUID·LDAP DN을 사람이 읽는 이름으로. AWS IAM Identity Center(SSO) federation 식별자 분리(`IdentityType.SSO_USER`/`SSO_GROUP` + 자홍 SSO 태그). 위험도(`risk_level`) 한국어 통일. AccessReview 위험 색띠 + AI 결정 카드. MyMond 빈 상태 CTA. 라우트별 코드 스플릿으로 첫 로드 1834KB → 684KB.
