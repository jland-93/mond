...
### 단기 (v0.2) — 완료 (Unreleased)

- [x] **SBOM 실 의존성 추출** — `package.json` · `package-lock.json` · `requirements.txt` · `go.mod` · `Dockerfile` 5종 파서
- [x] **SBOM Diff on PR** — `pull_request` 이벤트에 신규/제거/버전 변경 추출 + PR comment + Slack
- [x] **AI Insights RAG** — Asset · Finding · Policy · Knowledge 4 소스 + `[N]` inline citation
- [x] **비동기 스캔 큐 (Celery)** — `SCAN_QUEUE_ENABLED=true`로 enqueue
- [x] **OPA Rego 정책 평가** — backend Dockerfile에 OPA v1.x 번들 + `Policy.engine="opa"`
- [x] **자산 자동 동기화 (GitHub org)** — Admin → Connections에서 한 번에 일괄 등록. K8s / AWS Auto-scaling은 v0.3
- [x] **Webhook push → 스캐너 자동 선택** — 변경 파일 카테고리로 semgrep · trivy · nuclei 자동 라우팅
- [x] **CI 통합 패키지** — `.github/actions/mond-scan` 한 줄 통합
- [x] **Rate limiting · abuse protection** — Redis 기반 fixed-window + `Retry-After`
- [x] **AI 프롬프트 PII redaction** — 외부 LLM 호출 전 이메일/전화/RRN/AWS키/토큰 자동 마스킹 (E2E 암호화 대신 redaction 접근)
- [x] **GCP / Azure IAM grant 완성도 보강** — 멱등성 + etag 충돌 재시도
- [x] **UX·가시화 보강 (1차)** — IAM Explorer/Access Center/Access Review에서 ARN·UUID·LDAP DN을 사람이 읽는 이름으로. AWS IAM Identity Center(SSO) federation 식별자 분리(`IdentityType.SSO_USER`/`SSO_GROUP` + 자홍 SSO 태그). 위험도(`risk_level`) 한국어 통일. AccessReview 위험 색띠 + AI 결정 카드. MyMond 빈 상태 CTA. 라우트별 코드 스플릿으로 첫 로드 1834KB → 684KB.
- [x] **UX·가시화 보강 (2차)** — Scans 페이지에 webhook smart-router 자동 선택 결과(이유·카테고리 카운트) 노출. Findings drawer에 발견 자산 카드 + AI 인사이트(severity 재평가 화살표·confidence Progress bar). Assets owner inline edit('내 자산으로' 1클릭). Knowledge Hub 출처 명료화(📖/🤖). Regulations 시나리오 카드 그리드. Dashboard '다음 단계' 가이드 배너. AI Insights citation deep-link(`?focus=N`). Admin Policies engine 컬럼 + 임계치 색 dot. Settings 4 KPI 통합 상태 카드.
- [x] **UX·가시화 보강 (3차)** — AI Insights '이어서 묻기' follow-up chip(intent별 표준 + Claude `suggested_actions`). PolicySimulator에 engine 컬럼 + OPA deny 메시지 expand + 위험도 색띠. Admin Users에 MFA 등록 상태 컬럼(`mfa_enrolled`)·SSO 출처 색 구분·최근 로그인 상대 표기·본인 행 'you' 태그.

### v0.3 후보

- **자산 자동 동기화 확장** — Kubernetes 클러스터(namespace/pod), AWS Auto-scaling group, GitLab/Bitbucket org sync
- **AI 멀티 프로바이더 라우팅** — intent별로 다른 model (e.g. triage=Haiku, deep-explain=Sonnet)
- **SBOM CycloneDX 정식 출력** — lite stub 대체, vex 포함
- **MCP HTTP 마운트 안정화** — Claude Desktop/Code 외부 에이전트 도구화
- **다중 워크스페이스/조직 분리** — 사내 여러 팀 공유 인스턴스
- **감사 로그 검색 UI** — access_audit_logs 시계열 + 필터
- **한국 규제 인증 심사 패키지** — ISMS-P 자동 증빙 자료 출력
- **알림 라우팅 다채널** — Email/Teams/PagerDuty
- **온프레미스 LLM 게이트웨이 표준화** — 사내 vLLM 어댑터 + 토큰 사용량 추적
- **PR Bot — AI 분석 PR comment** — push 스캔 결과를 PR에 자동 코멘트
...
- **SBOM**: ~~v0.1: CycloneDX-lite stub~~ → **v0.2 해소**: 5종 ecosystem 실 파서 + PR diff
- **스캐너 실행**: ~~v0.1: 동기 인라인~~ → **v0.2 해소**: Celery 큐 옵션
- **AI Insights**: ~~v0.1: RAG 미적용~~ → **v0.2 해소**: 4 소스 RAG + inline citation. **hallucination 위험 인지 + 인간 검토** 권장은 유지. AI 생성 카드는 ADMIN 전용
- **IAM 어댑터**: ~~v0.1: GCP/Azure 보강 중~~ → **v0.2 해소**: 5종 모두 grant/revoke 멱등성 보장. capability API는 `ready`/`coming_soon`/`demo`를 그대로 노출
